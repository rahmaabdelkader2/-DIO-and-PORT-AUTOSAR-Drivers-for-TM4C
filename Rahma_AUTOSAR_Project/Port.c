...
void Port_Init(const Port_ConfigType* ConfigPtr)
{
  ...
  for(pin_ID = 0; pin_ID < PORT_PINS_NUM; pin_ID++)
  {
    ...
    /* 4. Set pin direction */
    if(PORT_PIN_IN == Port_configurations[pin_ID].direction)
    {
      /* Set pin as input */
    ...
    if(PORT_PIN_IN == Port_configurations[pin_ID].direction)
    {
      if(PULL_UP == Port_configurations[pin_ID].pin_internalResistor)
      {
    ...
    if(PORT_PIN_OUTPUT == Port_configurations[pin_ID].direction)
    {
      if(STD_HIGH == Port_configurations[pin_ID].initial_value)
      {
